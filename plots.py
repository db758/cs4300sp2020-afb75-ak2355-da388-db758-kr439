...
def makePlot1(column):
    d = df.groupby(column)
    plt.figure(figsize=(12,8))
    d.size().sort_values(ascending=False).head(20).plot.bar()
    plt.xticks(rotation=50)
    plt.xlabel("Name of " + column)
    plt.ylabel("Number of Liked Movies")
    plt.savefig("myPlot1"+".png", bbox_inches='tight')
    plt.show()
    plt.close()

def makePlot2(column):
    gen = {}
    for index, row in df.iterrows():
        for val in [x.strip(" []") for x in row[column].split(',')]:
            if val not in gen:
                gen[val] = 1
            else:
                gen[val] += 1
    s = sorted(gen.items(), key=operator.itemgetter(1), reverse = True)
    g = collections.OrderedDict(s)

    plt.bar(*zip(*g.items()))
    plt.xticks(rotation=100)
    plt.xlabel("Name of " + column)
    plt.ylabel("Number of Liked Movies")
    plt.savefig("myPlot2"+".png", bbox_inches='tight')
    plt.show()
    plt.close()
